...
1. The spectrum of uncertainty: from "certain" to "deep uncertainty"
   1. Certain: A situation where the outcome is known with perfect precision.
      1. Example: The probability of a fair coin landing on heads is exactly 0.5.
   1. Objective probabilities: Uncertainty that can be quantified using empirical data or well-established models.
      1. Example: The probability of a car accident can be estimated based on historical data and actuarial models.
   1. Subjective probabilities: Uncertainty that is quantified using expert judgment and subjective beliefs.
      1. Example: The probability of a new technology achieving widespread adoption within the next decade may be estimated by experts based on their knowledge and intuition.
   1. Deep uncertainty: Situations where probabilities cannot be reliably quantified, and even the range of possible outcomes may be unknown.
      1. Example: The long-term impact of artificial intelligence on society and the economy is deeply uncertain, as it depends on complex and unpredictable technological, social, and political factors.
      1. No one disputes that research on climate change and climate impacts is subject to deep unccertanities! The question is what to do about it.

1. The impact of uncertainty on decision-making approaches
   1. Probabilities are required for many traditional decision-making tools, such as cost-benefit analysis and risk assessment.
...
1. Implications for decision-making under uncertainty
   1. Decision-makers should be aware of the different types of uncertainty and how they influence the choice of decision-making tools and strategies.
   1. In situations with deep uncertainty, traditional optimization approaches may be misleading, and more emphasis should be placed on identifying strategies that are robust to a wide range of possible futures.
   1. Even in cases of deep uncertainty, attempting to quantify key uncertainties using expert judgment can provide valuable insights and help to inform risk management decisions.
   1. Effective decision-making under uncertainty requires a combination of quantitative analysis, expert judgment, and a willingness to adapt strategies as new information becomes available.
...
2. Optimal policies based on point estimates of the future are brittle and fail to support consensus among stakeholders with divergent expectations.
...
3. Instead of seeking optimal policies, Lempert proposes that we should focus on identifying robust strategies that perform reasonably well across a wide range of plausible futures.
...
4. Lempert advocates [exploratory modeling](./06-scenario-analysis.qmd) to help identify [robust](./10-robustness.qmd) strategies under deep uncertainty.
...
2. The IPCC's failure to assign probabilities to its emissions scenarios and climate model projections leads to confusion and allows for misinterpretation by those seeking to highlight extreme outcomes.
...
3. Schneider proposes that probabilistic assessments should be attempted for each step in the climate change modeling chain, from emissions scenarios to carbon cycle modeling, climate sensitivity, and impacts.
...
4. Schneider illustrates the importance of considering probabilities by showing how different assumptions about the likelihood of IPCC emissions scenarios and climate model sensitivities can lead to very different estimates of the probability of exceeding key temperature thresholds.
...
1. What types of information is actually useful to decision-makers? How does this depend on who the end user is?
...
1. Beyond climate change, can you think of other decision contexts characterized by deep uncertainty where you might favor one approach over the other, or attempt to combine them?

## Debrief and Wrap-up

1. Our context is about whether or not it is helpful to assign probabilities to IPCC scenarios. Ultimately this is a debate about the role of the IPCC -- is it to do as consolidate all available information for decision-makers? Or is it an assessment of the things we really know, which requires leaving everything we're not sure about as "what-ifs"
